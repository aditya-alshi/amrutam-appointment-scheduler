...
import { dayOfWeekMap, Days } from "../types/doctorTypes";
...
const idealDaysSelector = createSelector(
  (state: RootState) => {
    return state.appointments.currentDoctorId
  },
  (state: RootState) => state.appointments.doctors, 
  (currentDoctorId, doctorsArray) => doctorsArray.find(doc => doc.doctor_id === currentDoctorId)
)

export default function RawCalender() {
  let today = startOfToday();
  let [selectedDay, setSelectedDay] = useState(today);
  let [currentMonth, setCurrentMonth] = useState(format(today, "MMM-yyyy"));
  let firstDayCurrentMonth = parse(currentMonth, "MMM-yyyy", new Date());

  let currnetDoctorInfo = useSelector(idealDaysSelector);

  let selectedIdealDays = currnetDoctorInfo?.doctor_working_days.map((day) => dayOfWeekMap[day as Days]);
  console.log(selectedIdealDays)

  // const selecteDuration = useSelector((state: RootState) => state.setting.duration)
  const selecteDuration = currnetDoctorInfo?.doctor_appointment_duration ;
  console.log(selecteDuration)

  // const selectedSlots = useSelector((state: RootState) => state.setting.slots);
  const selectedSlots = currnetDoctorInfo?.doctor_time_slots;
  ...
  const allGeneratedAppointments = selectedSlots?.flatMap((appointment) => {
    ...
    let duration: number;
    if(selecteDuration){
       duration = parseInt(selecteDuration )
    }
     duration = 25
